...
router.post('/', async(req, res) => {
    ...
    try {
        ...
        const [insertResult] = await transaction.execute(
            'INSERT INTO person (id,name,alive,eatenBy) VALUES (NULL, ?, 1, NULL)', [name]);
        if (insertResult) {
            req.flash('success', 'Pessoa adicionada com sucesso!')
        } else {
            req.flash('error', 'Erro ao adicionar nova pessoa!')
        }
    } catch (error) {
        console.error(error)
        throw error;
...
router.delete('/:id', async(req, res) => {
    ...
    try {
        ...
        if (deleteResult) {
            ...
            req.flash('error', 'Erro ao remover pessoa!')
        }
    } catch (error) {
        console.error(error)
        throw error;
